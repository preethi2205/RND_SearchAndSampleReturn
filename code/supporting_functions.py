...
def create_output_images(Rover):
      ...
      if (Rover.total_time - Rover.map_time) >= 30:
          # For every 10 seconds
          Rover.old_perc_mapped = Rover.new_perc_mapped;
          Rover.new_perc_mapped = perc_mapped;
          Rover.map_time = Rover.total_time;
      
      SteerVel = np.mean(np.diff(Rover.SteerVel[0:6]));
      #(Rover.new_perc_mapped < 55) and
      no_new_area_mapped = (np.abs(Rover.new_perc_mapped - Rover.old_perc_mapped) <= 0.25); 
      rover_unstucking = (np.abs(Rover.total_time - Rover.map_time) <= 4);
      ...
      cv2.putText(map_add,"  Mode: "+str(Rover.mode), (0, 100), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
      cv2.putText(map_add,"  Rover.Is_Stuck: "+str(Rover.Is_Stuck), (0, 115), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)  
      cv2.putText(map_add,"  C2: "+str(rover_unstucking), (0, 130), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1) 
      cv2.putText(map_add,"  C3: "+str(rover_steer_not_changing), (0, 145), 
                  cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1) 
